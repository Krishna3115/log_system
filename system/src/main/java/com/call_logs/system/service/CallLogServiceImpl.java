...
import com.call_logs.system.dto.PaymentRequest;
...
public class CallLogServiceImpl implements CallLogService {
	 ...
	 // service/CallLogService.java
	    ...
	    public CallLog completeWork(Long id, String workDescription, Double totalBillAmount, String customerEmail) {
	        ...
	        callLog.setCustomerEmail(customerEmail);
	        callLog.setStatus(Status.BILL_PENDING);

	        // ✅ Save first
	        CallLog savedLog = callLogRepository.save(callLog);

	        // ✅ Send email after saving
	        sendBillEmail(savedLog);
	    ...
	    @Override
	    public List<CallLog> getPendingBills() {
	        return callLogRepository.findByStatus(Status.BILL_PENDING);
	    }

	    @Override
	    public CallLog payBill(Long id, PaymentRequest request) {
	        CallLog callLog = callLogRepository.findById(id)
	                .orElseThrow(() -> new RuntimeException("CallLog not found with id " + id));

	        double paid = request.getPaidAmount() != null ? request.getPaidAmount() : 0.0;
	        double total = callLog.getTotalBillAmount() != null ? callLog.getTotalBillAmount() : 0.0;
	        double pending = total - paid;

	        callLog.setPaidAmount(callLog.getPaidAmount() != null ? callLog.getPaidAmount() + paid : paid);
	        callLog.setPendingAmount(pending > 0 ? pending : 0.0);
	        callLog.setPaymentMethod(request.getPaymentMethod());

	        if (request.isFullPayment() || pending <= 0) {
	            callLog.setStatus(Status.PAID);
	        } else {
	            callLog.setStatus(Status.PARTIALLY_PAID);
	        }
	    ...
	    @Override
	    public List<CallLog> getPartiallyPaidBills() {
	        return callLogRepository.findByStatus(Status.PARTIALLY_PAID);
	    }
